...
interface RemarkEntry {
  condition: (d: RemarkContext) => boolean;
  pool: string[];
}
const REMARKS: Record<string, RemarkEntry> = {
  lateNight: {
...
export async function generateRemark(platform: string | null): Promise<string | null> {
  const now = new Date();
  const key = todayKey();
  ...
  const vars: Record<string, string> = {
    platform: ({ claude: "Claude", chatgpt: "ChatGPT", gemini: "Gemini" } as Record<string, string>)[maxPlatform] || maxPlatform,
